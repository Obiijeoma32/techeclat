...
function ApplicantProfile({ children }) {
  const menuList = [
    {
      path: "/homeforapplicant",
      name: "Home",
      icon: (
      ...
      <div className=" w-[170px] 820xxl:hidden fixed  h-[500px] border-r-[1px] border-r-[#F3F3F3]">
        {menuList.map((item, index) => (
          <NavLink
            to={item.path}
            key={index}
            className={({ isActive }) => (isActive ? " items-center text-[18px]  text-left font-[400] flex justify-between w-[300px] text-[#38761D]" : " items-center text-[18px]   text-left font-[400] flex justify-between w-[300px] text-[#38761d] ")}
          >
